/*
Prioridades
Este exemplo demonstra a prioridade das operações aritméticas no Portugol. As operações de multiplicação (*), divisão (/) e módula (%) têm prioridade sobre as 
operações de soma (+) e subtração (-). Além disso, o exemplo demonstra como os parênteses podem ser utilizados para alterar este prioridade, 
fazenda com que uma operação de soma ocorra antes de uma operação de multiplicação.
*/
programa {
  funcao inicio() {
    real resultado
    ...
    */
    ...
    /*
    *Neste exemplo, a operação de soma (+) será excutada primeiro,
